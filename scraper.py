...
def parse_flat_array(flat):
    ...
    for idx in appt_indices:
        ...
        try:
            ...
            # Keep original English status for dashboard
...
def analyze(appointments):
    ...
    # Today in Cairo timezone (UTC+2)
    today_str = (datetime.now(timezone.utc) + timedelta(hours=2)).strftime("%Y-%m-%d")

    today_appointments = []  # ALL of today's appointments
    ...
    for a in appointments:
        ...
        # Collect today's appointments separately (ALL of them)
        if start == today_str:
    ...
    return {
        ...
        "today_appointments": today_appointments,  # ALL today's appointments
    }
